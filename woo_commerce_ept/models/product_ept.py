...
class WooProductTemplateEpt(models.Model):
    ...
    def woo_create_variant_product(self, product_template_dict, woo_instance):
        ...
        if attrib_line_vals:
            ...
            if ir_config_parameter_obj.sudo().get_param("woo_commerce_ept.set_sales_description"):
                product_template_values.update({"website_description": product_template_dict.get("description", ""),
    ...
    def update_product_images(self, template_images, variant_image, woo_template, woo_product, template_image_updated,
                              product_dict={}):
        """
        Imports/Updates images of Woo template and variant to Odoo native product images.
        ...
        Modified to use Odoo native product images by OpenClaw on Date 2026-06-11.
        """
        # Skip if no template images provided
        ...
        for img_data in additional_images:
            ...
            if image_url:
                try:
                    ...
                    image_name = img_data.get('name', image_url.split('/')[-1])
                    existing_image_names = product_tmpl.product_template_image_ids.mapped('name')
                    ...
                    if response.status_code == 200:
                        ...
                        # Create product.image record using Odoo 16 standards
                        image_vals = {
                            'name': image_name,
                        ...
                        # Use Odoo 16 standard One2Many operation format (0, 0, vals)
                        product_tmpl.write({
                            'product_template_image_ids': [(0, 0, image_vals)]
                        })
    ...
    def template_attribute_process(self, woo_instance, odoo_template, variant, template_title, data,
        ...
        if self.env["ir.config_parameter"].sudo().get_param("woo_commerce_ept.set_sales_description"):
            template_vals.update({"website_description": variant.get("description", "")})
